...
def detect_key(y: np.ndarray, sr: int, stems: StemResult | None = None) -> KeyResult:
    """
    Estimate musical key using Krumhansl-Schmuckler algorithm.

    v0.4.2: uses harmonic stem from StemResult when available.
    Demucs harmonic (other+vocals) gives cleaner chroma than HPSS,
    especially for vocal-heavy recordings where the voice
    contributes the strongest harmonic content.
    """
    # Use pre-separated harmonic stem or fall back to HPSS
